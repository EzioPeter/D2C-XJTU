...
from d2c.networks_and_utils_for_agent.darc_nets_utils import SamplerPolicy, ActorNetwork
...
class DARCAgent(BaseAgent):
    ...
    def _optimize_step(self, batch: Tuple[Dict, Dict]) -> Dict:
        ...
        _real_batch, _sim_batch = batch
        self.new_actions, self.log_pi = self._p_fn(_sim_batch['s1'])
        d_info = self._optimize_dsa_dsas()
        alpha_loss, alpha_info = self._build_alpha_loss(_sim_batch)
        if self._global_step % self._update_actor_freq == 0:
            p_loss, self._p_info = self._build_p_loss(_sim_batch)
        q_loss, q_info = self._build_q_loss(_sim_batch)
